...
echo "Deploying Hunyuan on NVIDIA H100 GPU"
echo "===================================="
...
if ! nvidia-smi > /dev/null 2>&1; then
  echo "ERROR: NVIDIA GPU not detected! Please run h100-direct-nvidia.sh first and reboot."
  exit 1
fi
...
# Create Docker configuration optimized for H100
echo "Creating H100-optimized Docker configuration..."
cat > docker-hunyuan-h100.yml << EOL
services:
  hunyuan-api:
    build:
...
# Create optimized Dockerfile for H100
echo "Creating optimized Dockerfile for H100..."
mkdir -p hunyuan-docker
cat > hunyuan-docker/Dockerfile.h100 << EOL
...
RUN pip install xformers==0.0.23 flash-attn==2.3.6 triton==2.1.0 
...
# Create download_weights.py that points to right repository
...
# Create placeholder hunyuan_api.py if it doesn't exist
...
echo "Building and starting the Hunyuan container for H100..."
