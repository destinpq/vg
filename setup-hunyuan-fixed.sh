...
# Skip GPU check temporarily
echo "Checking for NVIDIA GPU (skipping for now since we're having issues)..."
...
# Display GPU info
echo "Attempting to get GPU info:"
nvidia-smi || echo "nvidia-smi failed, but continuing anyway..."

# Create Docker configuration
echo "Creating Docker configuration..."
mkdir -p hunyuan-docker
DOCKER_COMPOSE_FILE="$(pwd)/docker-hunyuan.yml"
...
if [ ! -f "$DOCKER_COMPOSE_FILE" ]; then
  echo "ERROR: Failed to create Docker Compose file at $DOCKER_COMPOSE_FILE"
  exit 1
else
  echo "Successfully created Docker Compose file"
  ls -la "$DOCKER_COMPOSE_FILE"
fi

# Create Dockerfile
DOCKERFILE="$(pwd)/hunyuan-docker/Dockerfile"
...
if [ ! -f "$DOCKERFILE" ]; then
  echo "ERROR: Failed to create Dockerfile at $DOCKERFILE"
  exit 1
else
...
ENTRYPOINT_SCRIPT="$(pwd)/hunyuan-docker/entrypoint.sh"
echo "Creating entrypoint script at: $ENTRYPOINT_SCRIPT"

cat > "$ENTRYPOINT_SCRIPT" << EOL
#!/bin/bash
set -e
...
nvidia-smi || echo -e "\${YELLOW}WARNING: nvidia-smi failed, but continuing with CPU fallback${NC}"
...
# Check if the file was created and make it executable
if [ ! -f "$ENTRYPOINT_SCRIPT" ]; then
  echo "ERROR: Failed to create entrypoint script at $ENTRYPOINT_SCRIPT"
  exit 1
else
  echo "Successfully created entrypoint script"
  chmod +x "$ENTRYPOINT_SCRIPT"
  ls -la "$ENTRYPOINT_SCRIPT"
fi

# Create download_weights.py
DOWNLOAD_SCRIPT="$(pwd)/hunyuan-docker/download_weights.py"
...
if [ ! -f "$DOWNLOAD_SCRIPT" ]; then
  echo "ERROR: Failed to create download script at $DOWNLOAD_SCRIPT"
  exit 1
else
  echo "Successfully created download script"
  ls -la "$DOWNLOAD_SCRIPT"
fi

# Create placeholder hunyuan_api.py
API_SCRIPT="$(pwd)/hunyuan-docker/hunyuan_api.py"
echo "Creating API script at: $API_SCRIPT"
...
if [ ! -f "$API_SCRIPT" ]; then
  echo "ERROR: Failed to create API script at $API_SCRIPT"
  exit 1
else
  echo "Successfully created API script"
  ls -la "$API_SCRIPT"
fi

echo "===================================================="
echo "Setup complete! To build and run the container, use:"
echo "docker-compose -f $(pwd)/docker-hunyuan.yml build"
echo "docker-compose -f $(pwd)/docker-hunyuan.yml up -d"
echo ""
echo "To check logs:"
echo "docker-compose -f $(pwd)/docker-hunyuan.yml logs -f"
echo "====================================================" 